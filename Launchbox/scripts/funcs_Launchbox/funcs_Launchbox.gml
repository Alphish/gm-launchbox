...
/// @desc Retrieves or declares a box with the given identifier. When given a Launchbox box instance, passes it as-is.
...
/// @desc Checks if a Launchbox box with the given identifier has been declared. Returns true for Launchbox box instances.
...
/// @desc Declares a Launchbox box with the given identifier if one already doesn't exist ("main" by default).
...
/// @desc Passes a Launchbox box instance, or retrieves a box with the given identifier; throws an exception if such a box doesn't exist.
/// @arg {String,Struct.LaunchboxBox} [box]     The box to pass or retrieve; "main" by default.
/// @returns {Struct.LaunchboxBox}
...
/// @desc Puts a callback into the main Launchbox box, to be executed on launch.
/// @arg {Function} callback        The callback to execute on launch.
function launchbox_put(_callback) {
    launchbox().put(_callback);
}

/// @desc Puts a callback into the given Launchbox box, to be executed on launch.
...
/// @desc Puts an instance into the main Launchbox box, to be created on launch with given pre-Create variables and post-Create logic.
...
/// @desc Puts an instance into the given Launchbox box, to be created on launch with given pre-Create variables and post-Create logic.
/// @arg {String,Struct.LaunchboxBox} box       The box to put the callback into.
/// @arg {Asset.GMObject} object                The object to create the instance of.
/// @arg {Struct} variables                     The variables set on the newly created instance.
/// @arg {Function} postcreate                  The logic to execute on the newly created instance, after its Create event.
...
/// @desc Clears all callbacks from one or more Launchbox boxes, while keeping them declared. If no boxes are given, the "main" box is cleared.
...
/// @desc Discards one or more Launchbox boxes along with their callbacks, so they're no longer declared. If no boxes are given, the "main" box is discarded.
...
/// @desc Launches one or more Launchbox boxes, executing all their callbacks and discarding them. Missing declarations will throw errors. If no boxes are given, the "main" box is launched.
...
/// @desc Attempts to launch one or more Launchbox boxes, executing all their callbacks and discarding them. Missing declarations are ignored. If no boxes are given, the "main" box launch is attempted.
